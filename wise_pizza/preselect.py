...
class HeuristicSelector:
    def __init__(self, max_cols: int = 300):
        self.max_cols = max_cols
        self.col_defs = []
        self.X = None

    def __call__(self, X, col_defs, weights, totals):
        ...
        if self.X.shape[1] > self.max_cols:
            ...
            seg_wgt = self.X.T @ weights
            seg_avg = (self.X.T @ totals) / seg_wgt
            avg = totals.sum() / weights.sum()
